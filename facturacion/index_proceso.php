...
if(isset($_POST["ingreso"]))
{
	...
	{
		$coneccion=mysql_connect("localhost", "appstc", "nVgXi3HT40") or die(mysql_error());
		mysql_select_db("appstc_ayaguna_jmp", $coneccion) or die(mysql_error());
		
		$consulta=mysql_query("SELECT COUNT(*) FROM fact_bancos") or die(mysql_error());
		$registro=mysql_fetch_row($consulta);

		if($registro[0]>=600) { echo "Hay demasiados registros en la Base de Datos"; die(); }
		
		$consulta=mysql_query("SELECT * FROM fact_bancos WHERE banco='$valor'") or die(mysql_error());
		$registro=mysql_fetch_row($consulta);
		
		if($registro) { echo "Ya existe tu nombre en la Base de Datos"; }
		else
		{
			mysql_query("INSERT into fact_bancos (banco) VALUES ('$valor')");
...
if(isset($_POST["busqueda"]))
{
	...
	if(validaBusqueda($valor))
	{
		...
		$consulta=mysql_query("SELECT banco FROM fact_bancos WHERE banco LIKE '".$valor."%' LIMIT 0, 22");
