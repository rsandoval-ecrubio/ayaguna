...
if(isset($_POST) and !empty($_POST)){
	...
	$qinventario = sprintf("CALL `GateOut`('%d', '%s', '%s');",$linea,$ini,$fin);
	$inventario->Consultar($qinventario);
	
	$mostrar = $inventario->Total;
	
	$recap20 = new MySQL(USERDB);
	$qrecap20 = sprintf("SELECT tequipos.tipo, COUNT(*) AS cantidad FROM inventario, tequipos WHERE inventario.delete = 0 AND inventario.linea = %d AND inventario.fdespims BETWEEN '%s' AND '%s' AND tequipos.id = inventario.tcont AND tequipos.tipo LIKE '2%%'GROUP BY inventario.tcont;",$linea,$ini,$fin);
	$recap20->Consultar($qrecap20);
	
	$recap40 = new MySQL(USERDB);
	$qrecap40 = sprintf("SELECT tequipos.tipo, COUNT(*) AS cantidad FROM inventario, tequipos WHERE inventario.delete = 0 AND inventario.linea = %d AND inventario.fdespims BETWEEN '%s' AND '%s' AND tequipos.id = inventario.tcont AND tequipos.tipo LIKE '4%%'GROUP BY inventario.tcont;",$linea,$ini,$fin);
...
<table cellpadding="0" class="listado" id="lista" >
  <caption>
    Movimientos de Salida | <?php echo $inventario->Total; ?> Equipos
    | <a href="../gateout/export_gate_out.php?linea=<?php echo $_SESSION['variables']['linea']; ?>&dia=<?php echo $_POST['ini'];?>&dia2=<?php echo $_POST['fin'];?>">Exportar</a>
  </caption>
  <thead>
    <tr>
      ...
      <th axis="string">EIR/IN</th>
      <th axis="date">Arribo</th>
      <th axis="date">Ingreso</th>
      <th axis="number">EIR/OUT</th>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td align="center"><?php echo $inventario->Resultado['eir_d']; ?></td>
