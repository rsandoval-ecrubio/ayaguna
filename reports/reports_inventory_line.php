...
$colname_existLinea = "-1";
if (isset($_POST['linea'])) {
  $colname_existLinea = $_POST['linea'];
}
mysql_select_db($database_conexion, $conexion);
$query_existLinea = sprintf("SELECT * FROM existenciagral WHERE linea = %s", GetSQLValueString($colname_existLinea, "int"));
$existLinea = mysql_query($query_existLinea, $conexion) or die(mysql_error());
$row_existLinea = mysql_fetch_assoc($existLinea);
$totalRows_existLinea = mysql_num_rows($existLinea);
...
<body>
      <h2>INVENTARIO GENERAL - LINEA: <?php echo $row_existLinea['nlinea']; ?></h2>
    <form id="form1" name="form1" method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
      <label>Line:
        <select name="linea" id="linea">
          <option value="-1" <?php if (!(strcmp(-1, $_SESSION['linea']))) {echo "selected=\"selected\"";} ?>>Seleccion</option>
          <?php
do {  
?>
          <option value="<?php echo $row_line['id']?>"<?php if (!(strcmp($row_line['id'], $_SESSION['linea']))) {echo "selected=\"selected\"";} ?>><?php echo $row_line['nombre']?></option>
          <?php
...
<br />
<?php if ($totalRows_existLinea > 0) { // Show if recordset not empty ?>
  ...
  <div id="export"><a href="../export/export_reports_inventory_line.php?linea=<?php echo $_SESSION['linea']; ?>">exportar</a></div>
  <table align="left" cellpadding="0" id="pf_sortableTable1" >
    <caption>
      Inventario Linea
    | 
    <?php echo $totalRows_existLinea ?>
    </caption>
    <thead>
      <tr>
    ...
    <tbody>
      <?php do { ?>
        <tr>
          <td class="rightAlign"><?php echo $row_existLinea['id']; ?></td>
          <td><?php echo $row_existLinea['contenedor']; ?></td>
          <td><?php echo $row_existLinea['tipo']; ?></td>
          <td><?php estatus($row_existLinea['estatus']); ?></td>
          <td><?php condicion($row_existLinea['condicion']); ?></td>
          <td><?php echo $row_existLinea['buque']; ?></td>
          <td><?php echo $row_existLinea['viaje']; ?></td>
          <td align="center"><?php echo $row_existLinea['fda']; ?></td>
          <td align="center"><?php echo $row_existLinea['fdm']; ?></td>
          <td align="center"><?php echo $row_existLinea['frd']; ?></td>
          <td class="rightAlign"><?php echo $row_existLinea['precinto']; ?></td>
          <td class="rightAlign"><?php echo $row_existLinea['bl']; ?></td>
          <td class="rightAlign"><?php echo $row_existLinea['eir_r']; ?></td>
          <td class="rightAlign"><?php echo $row_existLinea['patio']; ?></td>
          <td class="rightAlign"><?php alarmapais($row_existLinea['dpais']); ?></td>
          <td class="rightAlign"><?php alarma($row_existLinea['dpatio']); ?></td>
        </tr>
        <?php } while ($row_existLinea = mysql_fetch_assoc($existLinea)); ?>
    </tbody>
    <tfoot>
    </tfoot>
  </table>
