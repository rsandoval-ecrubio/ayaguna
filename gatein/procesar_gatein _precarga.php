...
if($preContenedor->total > 0){
	...
	if($arribo->total > 0){
		...
		if($verificar->total > 0){
			//-FIN-Verificar que no este registrado el contenedo
			...
			$Qgatein = sprintf("
			INSERT INTO inventario (linea,buque,viaje,tcont,contenedor,fdb,fdm,frd,eir_r,fact,paset,`status`,condicion,bl,patio,consignatario,obs)
			VALUES (%d,%d,%d,%d,'%s','%s','%s','%s',%d,%d,%d,%d,%d,'%s',%d,'%s')",
			$linea,$buque,$viaje,$tipo,$contenedor,$fdb,$fdm,$frd,$eir_r,$fact,$pase,$estatus,$condicion,$bl,$patio,$consignatario,$observacion);
